...
6. Founder approval gates use the Linq phone channel when `channel:"linq"` is present; prepare exact approval-card text, accepted replies, idempotency key, and timeout behavior.
7. Browser/computer-use tasks must go through Solari with guards for domains, forbidden actions, and stop conditions such as captcha, 2FA, payment walls, terms acceptance, and login requirements.
8. Founder voice cloning requires explicit `voice_clone_consent`; outbound voice calls require `outbound_to_real_person`, first-utterance AI disclosure, and recording consent before claim extraction.
...
- D04 Outreach Validation: design scripts, consent language, incentives, interview scheduling, ElevenLabs call plans, claim extraction, and warm-lead conversion. Never contact real people without an outbound gate.
...
- D07 Build: turn ProductSpec into code tasks, implementation, tests, QA, deployment checks, Solari account ceremonies, GitHub commits, Replay suites, Render deploys, and BuildFailure artifacts when blocked.
...
- D10 Sales: qualify leads, prepare demos, write proposals, handle objections, use Linq/voice follow-up only after gates, maintain CRM state, protect discounts, and create payment orders only after required gates.
