...
const modules: Record<string, RealModule> = { apify, band, composio, dodo, elevenlabs, github, linq, pioneer, render, replay, solari, stripe, terac, whop };
const vendorByTool: Record<string, string> = { 'stripe.create_payment_link':'stripe','render.deploy':'render','elevenlabs.tts':'elevenlabs','composio.gmail_send':'composio','linq.send_card':'linq','terac.post_requisition':'terac','whop.create_checkout':'whop','dodo.create_checkout':'dodo','apify.run_actor':'apify','solari.browse':'solari','replay.run_suite':'replay','band.publish':'band','github.push':'github','pioneer.classify':'pioneer' };
export async function runRealTool(tool_name: string, args: unknown, degraded: (reason: string) => void, fallback: () => Promise<unknown>): Promise<unknown> { const vendor = vendorByTool[tool_name]; const mod = vendor ? modules[vendor] : undefined; if (!mod) return fallback(); if (!mod.hasKey()) { degraded(`missing ${vendor} api key`); return fallback(); } return mod.run(args); }
